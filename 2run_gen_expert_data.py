...
# para_dir = os.path.join(abs_dir, "_data/experts-new/", env_name, 'retry', "params.pkl")
para_dir = os.path.join(abs_dir, "_data/experts-new/", env_name, "params.pkl")
data = torch.load(para_dir)
...
demo_dir = os.path.join(abs_dir, "_data/demo/")
...
try:
    ...
    assert len(demonstrations) == NUM_DEMO, f'{NUM_DEMO} trajectories are needed, but only got {len(demonstrations)}'
