...
class CEIL(base.DemonstrationAlgorithm[types.Transitions]):
    ...
    def get_action(self, obs):
        ...
        action_out = self.actor(obs_tensor, self.context.to(self.device))[0]
        # SpikeF.reset_net(self.actor)
        return action_out
    ...
    def policy_evaluate(self, n_rollout_steps: int, n_rollout: int)->None:
        ...
        # self.logger.record("evaluate/ep_len_std", self.evaluate_stats["len_std"])
            ...
            self.logger.record("evaluate/ep_len_mean", self.evaluate_stats["len_mean"])
            self.logger.record("evaluate/ep_len_std", self.evaluate_stats["len_std"])
            torch.save(self.actor.state_dict(), os.path.join(self.pth_path, f'actor/actor_{n_rollout}.pth'))
            torch.save(self.encoder.state_dict(), os.path.join(self.pth_path, f'encoder/encoder_{n_rollout}.pth'))
...
class BaselineTrainer():
    def __init__(
            ...
            *,
            ...
            # disc_net,
            c_logger_folder: str,
            allow_variable_horizon: bool = True,
            replay_buffer_capacity: int = 200,
            algo_n_steps = 1,
            rollout_env_num: int = 10,
            eval_env_num: int = 5,
            eval_src_num: int = 3,
        ...
        # self.venv_eval = wrappers.BufferingWrapper(self.target_venv)
        # self.venv_eval_source = wrappers.BufferingWrapper(make_vec_env(source_env_name, n_envs=3, rng=rng))
        env_wrapper = [lambda env, _: wrappers.RolloutInfoWrapper(env)]
        self.source_venv_rollout = make_vec_env(source_env_name, n_envs=rollout_env_num, post_wrappers=env_wrapper, rng=rng, max_episode_steps=1000 if "heetah" not in self.source_env_name else 200)
        self.target_venv = make_vec_env(target_env_name, n_envs=eval_env_num, post_wrappers=env_wrapper, rng=rng) #, max_episode_steps=1000)
        self.source_venv_eval = make_vec_env(source_env_name, n_envs=eval_src_num, post_wrappers=env_wrapper, rng=rng)
        # self.venv_train = wrappers.BufferingWrapper(self.source_venv_rollout)
        # self.venv_eval = wrappers.BufferingWrapper(self.target_venv)
        # self.venv_eval_source = wrappers.BufferingWrapper(self.source_venv_eval)
        ...
        if use_pretrain:
            print(f'Downloaded expert policy only support v1 environment, HopperV2-->HopperV1')
            expert_env_name ={"Hopper-v2": "seals/Hopper-v1", 
                              "HalfCheetah-v2": "seals/HalfCheetah-v1", 
                              "Walker2d-v2": "seals/Walker2d-v1", 
                              "Ant-v2": "seals/Ant-v1"}[self.source_env_name]
            expert = load_policy(
                "ppo-huggingface",
                organization="HumanCompatibleAI",
                env_name=expert_env_name,
                venv=self.source_venv_rollout
            )
            demonstrations = rollout.rollout(
                expert,
                self.source_venv_rollout,
                rollout.make_sample_until(min_timesteps=None, min_episodes=60),
                rng=rng
            )
            print('Use pretrained expert to collect new trajectories!')
        if 'gail' in algo.lower():
            reward_net = BasicRewardNet(
                observation_space = self.source_venv_rollout.observation_space,
                action_space = self.source_venv_rollout.action_space,
                normalize_input_layer = RunningNorm,
                use_action= False if self.demo is 'lfo' else True
            )
            self.learner = PPO(
                env=self.source_venv_rollout,
                policy=MlpPolicy,
                batch_size=gen_batch_size,
                n_steps=algo_n_steps,
                ent_coef=0.0,
            ...
            self.trainer = GAIL(
                demonstrations=demonstrations,
                demo_batch_size=1024, # len(demonstrations) >= batch_size
                venv=self.source_venv_rollout,
    ...
    def set_seed(self, seed) -> None:
        set_random_seed(seed, using_cuda=True)
        self.source_venv_rollout.seed(seed)
        self.target_venv.seed(seed)
        self.source_venv_eval.seed(seed)
        self.source_venv_rollout.action_space.seed(seed)
        self.target_venv.action_space.seed(seed)
        self.source_venv_eval.action_space.seed(seed)
    ...
    def train_steps(self, total_steps_per_train) -> None:
        self.trainer.train(total_steps_per_train)
    
    def policy_evaluate(self, num_ep_per_eval: int, non_average: bool=True):
        eval_dict = {}
        target_rewards, target_length = evaluate_policy(
            self.learner,
            self.target_venv,
            num_ep_per_eval,
            return_episode_rewards=non_average
        )
        eval_dict['tar_rws'] = target_rewards
        eval_dict['tar_len'] = target_length
        if self.source_env_name != self.target_env_name:
            source_rewards, source_length = evaluate_policy(
                self.learner,
                self.source_venv_eval,
                num_ep_per_eval,
                return_episode_rewards=non_average
            )
            eval_dict['src_rws'] = source_rewards
            eval_dict['src_len'] = source_length
        
        return eval_dict
        

    def policy_evaluate_2(self, n_rollout_steps: int, n_rollout: int)->None:
        if n_rollout % 1000 == 0:
            self.learner.eval()
            last_obs = self.venv_eval.reset()
            n_steps = 0
