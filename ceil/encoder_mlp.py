...
class TrajEncoderSpike(TrajEncoder):
    def __init__(self,
                 horizon,
                 transition_dim,
                 context_dim,
        ...
        super().__init__(horizon,
                         ...
                         hidden_size,
                         ensemble_num)
        # self.enc_1 = layer.Linear(self.horizon*(self.obs_dim+1), hidden_size)
        # self.enc_2 = layer.Linear(hidden_size, hidden_size)
        # self.enc_3 = layer.Linear(hidden_size, hidden_size)
        # self.enc_4 = layer.Linear(hidden_size, self.context_dim)
        self.enc = nn.Sequential(
            layer.Linear(self.horizon*(self.obs_dim+1), hidden_size),
            self.node(surrogate_function = surrogate.ATan()),
            layer.Linear(hidden_size, hidden_size),
            self.node(surrogate_function = surrogate.ATan()),
            layer.Linear(hidden_size, hidden_size),
            self.node(surrogate_function = surrogate.ATan()),
            layer.Linear(hidden_size, self.context_dim)
        )

        # self.dec_obs_1 = layer.Linear(self.context_dim+self.embt_dim+self.obs_dim+1, hidden_size)
        # self.dec_obs_2 = layer.Linear(hidden_size, hidden_size)
        # self.dec_obs_3 = layer.Linear(hidden_size, hidden_size)
        # self.dec_obs_4 = layer.Linear(hidden_size, self.obs_dim+1)
        self.dec_obs = nn.Sequential(
            layer.Linear(self.context_dim+self.embt_dim+self.obs_dim+1, hidden_size),
            self.node(surrogate_function = surrogate.ATan()),
            layer.Linear(hidden_size, hidden_size),
            self.node(surrogate_function = surrogate.ATan()),
            layer.Linear(hidden_size, hidden_size),
            self.node(surrogate_function = surrogate.ATan()),
            layer.Linear(hidden_size, self.obs_dim+1)
        )        

        # self.act_1 = layer.Linear(self.obs_dim*2, hidden_size)
        # self.act_2 = layer.Linear(hidden_size, hidden_size)
        # self.act_3 = layer.Linear(hidden_size, hidden_size)
        # self.act_4 = layer.Linear(hidden_size, self.act_dim)
        self.act = nn.Sequential(
            layer.Linear(self.obs_dim*2, hidden_size),
            self.node(surrogate_function = surrogate.ATan()),
            layer.Linear(hidden_size, hidden_size),
            self.node(surrogate_function = surrogate.ATan()),
            layer.Linear(hidden_size, hidden_size),
            self.node(surrogate_function = surrogate.ATan()),
            layer.Linear(hidden_size, self.act_dim)
        )
        for model in self.transi_models:
            for idx, layer_pre in enumerate(model):
                if isinstance(layer_pre, nn.Linear):
                    model[idx] = layer.Linear(layer_pre.in_features, layer_pre.out_features)
                elif isinstance(layer_pre, nn.ReLU):
                    model[idx] = self.node(surrogate_function = surrogate.ATan())
        if use_cupy:
    ...
    def forward(self, inputx, inputts,
        ...
        if obsp:
            ...
            x = torch.cat([x, ts, xi0], -1)
            x_seq = x.unsqueeze(0).repeat(self.T, *[1 for i in range(len(x.shape))])
            obs = self.dec_obs(x_seq).mean(0)

        act = None
        if actp:
            act_x = torch.cat([inputx[:, :-1, :self.obs_dim], inputx[:, 1:, :self.obs_dim]], -1)
            act_seq = act_x.unsqueeze(0).repeat(self.T, *[1 for i in range(len(act_x.shape))])
            act = self.act(act_seq).mean(0)

        transiss = []
        if transip:
            for transi_model in self.transi_models:
                transiss.append(transi_model(inputx.unsqueeze(0).repeat(self.T, *[1 for i in range(len(inputx.shape))])).mean(0))

        # functional.reset_net(self.enc)
        # functional.reset_net(self.dec_obs)
        # functional.reset_net(self.act)
